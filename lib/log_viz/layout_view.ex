defmodule LogViz.LayoutView do
  @moduledoc false
  use LogViz.Web, :html
  ...
  defp asset_path(conn, asset) when asset in [:css, :js] do
    hash = LogViz.Assets.current_hash(asset)

    if function_exported?(conn.private.phoenix_router, :__log_viz_prefix__, 0) do
      prefix = conn.private.phoenix_router.__log_viz_prefix__()
