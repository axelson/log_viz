defmodule LogViz.Router do
  @moduledoc """
  Provides LiveView routing for LogViz.

  Code inspired by Phoenix.LiveDashboard.Router
  """

  @doc """
  Defines a LogViz route.

  It expects the `path` the LogViz will be mounted at
  and a set of options. You can then link to the route directly:

      <a href={~p"/logs"}>LogViz</a>
  ...
  ## Options
    ...
    * `:home_app` - A tuple with the app name and version to show on
      the home page. Defaults to `{"LogViz", :log_viz}`
    ...
    * `:on_mount` - Declares a custom list of `Phoenix.LiveView.on_mount/1`
      callbacks to add to the log visualiztion's `Phoenix.LiveView.Router.live_session/3`.
    ...
    * `:allow_destructive_actions` - When true, allow destructive actions directly
      ...
      available in the log visualization:
  ...
  defmacro log_viz(path, opts \\ []) do
      ...
      quote bind_quoted: binding() do
        scope path, alias: false, as: false do
          {session_name, session_opts, route_opts} =
            LogViz.Router.__options__(opts)
          ...
          live_session session_name, session_opts do
            # LogViz assets
            get "/css-:md5", LogViz.Assets, :css, as: :log_viz_asset
            get "/js-:md5", LogViz.Assets, :js, as: :log_viz_asset

            # All helpers are public contracts and cannot be changed
            live "/", LogViz.LogsLive, :logs, route_opts
            # live "/", LogViz.PageLive, :home, route_opts
            # live "/:page", LogViz.PageLive, :page, route_opts
            # live "/:node/:page", LogViz.PageLive, :page, route_opts
          end
        end
      end
    ...
    if Code.ensure_loaded?(Phoenix.VerifiedRoutes) do
      quote do
        ...
        unless Module.get_attribute(__MODULE__, :log_viz_prefix) do
          @log_viz_prefix Phoenix.Router.scoped_path(__MODULE__, path)
                                 |> String.replace_suffix("/", "")
          def __log_viz_prefix__, do: @log_viz_prefix
        end
      end
    else
      scope
  ...
  defp expand_alias({:__aliases__, _, _} = alias, env),
    do: Macro.expand(alias, %{env | function: {:log_viz, 2}})
  ...
  def __options__(options) do
    ...
    {
      options[:live_session_name] || :log_viz,
      [
        session: {__MODULE__, :__session__, session_args},
        root_layout: {LogViz.LayoutView, :dash},
      ...
      other ->
        msg =
          "invalid value in :additional_pages, " <>
            ...
            "is an atom and the module implements LogViz.PageBuilder, got: "
  ...
  def __session__(
        ...
        _request_logger,
        _ecto_repos,
        _ecto_psql_extras_options,
        _ecto_mysql_extras_options,
        _ecto_sqlite3_extras_options,
        csp_nonce_assign_key
      ) do
    # ecto_session = %{
    ...
    {pages, requirements} =
      [
        home: {LogViz.LogsLive, %{env_keys: env_keys, home_app: home_app}},
        # os_mon: {LogViz.OSMonPage, %{}},
        # memory_allocators: {LogViz.MemoryAllocatorsPage, %{}}
      ]
      # |> Enum.concat(metrics_page(metrics, metrics_history))
      # |> Enum.concat(request_logger_page(conn, request_logger))
      # |> Enum.concat(
      #   applications: {LogViz.ApplicationsPage, %{}},
      #   processes: {LogViz.ProcessesPage, %{}},
      #   ports: {LogViz.PortsPage, %{}},
      #   sockets: {LogViz.SocketsPage, %{}},
      #   ets: {LogViz.EtsPage, %{}},
      #   ecto_stats: {LogViz.EctoStatsPage, ecto_session}
      # )
  ...
  #   [metrics: {LogViz.MetricsPage, session}]
  # end

  # defp request_logger_page(_conn, {false, _}), do: []

  # don't include request logger
  # defp request_logger_page(_conn, _), do: []

  # defp request_logger_page(conn, {true, cookie_domain}) do
  #   session = %{
  #     request_logger: LogViz.RequestLogger.param_key(conn),
  #     cookie_domain: cookie_domain
  #   }

  #   [request_logger: {LogViz.RequestLoggerPage, session}]
  # end
